...
def _apply_environment_overrides(config: dict, environment: str) -> dict:
    ...
    if not isinstance(env_map, dict):
        raise ValueError("Config key 'environments' must be a JSON object.")
    ...
    if not isinstance(env_override, dict):
        raise ValueError(f"Environment override for '{environment}' must be a JSON object.")
...
def _validate_policy_thresholds(config: dict, policy: str) -> None:
    ...
    if not isinstance(policy_config, dict):
        raise ValueError(f"Config key '{policy}' must be a JSON object.")
    ...
    if not isinstance(warn_value, (int, float)):
        raise ValueError(f"Config key '{policy}.warn_increase_pct' must be numeric.")
    if not isinstance(block_value, (int, float)):
        raise ValueError(f"Config key '{policy}.block_increase_pct' must be numeric.")
    if float(warn_value) < 0 or float(block_value) < 0:
        raise ValueError(f"Config key '{policy}' thresholds must be >= 0.")
...
def _validate_drift_thresholds(config: dict) -> None:
    ...
    if not isinstance(drift, dict):
        raise ValueError("Config key 'drift_policy' must be a JSON object.")
    ...
    if not isinstance(length_delta, (int, float)):
        raise ValueError("Config key 'drift_policy.warn_length_delta_pct' must be numeric.")
    if float(length_delta) < 0:
        raise ValueError("Config key 'drift_policy.warn_length_delta_pct' must be >= 0.")

    if not isinstance(short_ratio, (int, float)):
        raise ValueError("Config key 'drift_policy.warn_short_ratio' must be numeric.")
    if not 0 <= float(short_ratio) <= 1:
        raise ValueError("Config key 'drift_policy.warn_short_ratio' must be in [0, 1].")

    if not isinstance(min_similarity, (int, float)):
        raise ValueError("Config key 'drift_policy.warn_min_similarity' must be numeric.")
    if not 0 <= float(min_similarity) <= 1:
        raise ValueError("Config key 'drift_policy.warn_min_similarity' must be in [0, 1].")
